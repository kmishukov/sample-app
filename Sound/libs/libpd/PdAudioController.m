...
@property (nonatomic, readwrite) int blocksPerBuffer;
...
// make sure the resulting blocks value is not halved.
- (PdAudioStatus)configureBlocksPerBuffer:(int)blocksPerBuffer {
	int numberFrames = BLOCK_SIZE * blocksPerBuffer;
	...
	int tpb = self.blocksPerBuffer;
	if (tpb != blocksPerBuffer) {
		AU_LOG(@"*** WARNING *** could not set IO buffer duration to match %d blocks, got %d blocks instead", blocksPerBuffer, tpb);
...
- (int)blocksPerBuffer {
	...
	_blocksPerBuffer = round((asBufferDuration * self.sampleRate) /
	                        (NSTimeInterval)BLOCK_SIZE);
	return _blocksPerBuffer;
}
